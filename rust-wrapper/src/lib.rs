...
impl _RustRobot {
    ...
    fn forward_kinematics(&self, q: [f64; 6]) -> ([[f64; 3]; 3], [f64; 3]) {
        if !self.kin_set && !self.is_hardcoded {
            panic!("Kinematics must be set before calling forward kinematics",);
        } else if self.is_hardcoded {
            panic!("Forward kinematics not implemented for hardcoded solvers",);
        }
...
// Set the kinematics for the robot
#[no_mangle]
pub extern "C" fn set_robot_kinematics(
    robot_in: *mut _RustRobot,
    h_vals: *const f64,
    p_vals: *const f64,
) {
    let mut robot = unsafe { Box::from_raw(robot_in) };
...
pub extern "C" fn get_robot_ik(
    robot_in: *mut _RustRobot,
    rot_matrix_ptr: *const f64,
    t_vec_ptr: *const f64,
    return_q: *mut *mut f64,
    return_is_ls: *mut *mut bool,
...
#[no_mangle]
pub extern "C" fn forward_kinematics(
    robot_in: *mut _RustRobot,
    q_in: *const f64,
    rot_matrix_out: *mut f64,
    t_vec_out: *mut f64,
) {
    let robot = unsafe { Box::from_raw(robot_in) };
    let mut q = [0.0; 6];
    for i in 0..6 {
